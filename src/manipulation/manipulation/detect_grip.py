...
class ChickenChaser(Node):
    ...
    def get_stable_angle(self, mask_coords):
        ...
        # PCACompute는 분산이 큰 순서대로 벡터를 반환합니다.
        mean, eigenvectors = cv2.PCACompute(pts, mean=None)
        
        # 1. 항상 '가장 긴 축(주축)'인 eigenvectors[0]을 기준으로 각도를 잡습니다.
        # 이렇게 하면 90도씩 튀는 현상을 최소화할 수 있습니다.
        primary_axis = eigenvectors[0]
        angle = np.degrees(np.arctan2(primary_axis[1], primary_axis[0]))
        
        # 2. 치킨의 긴 방향이 아니라 '옆구리(짧은 쪽)'를 잡아야 하므로 90도를 더해줍니다.
        # (만약 90도 틀어져서 잡는다면 이 값을 0으로 바꾸거나 조정하세요)
        angle += 90.0

        # 3. -90 ~ 90도 사이로 정규화 (그리퍼 대칭성 반영)
    ...
    def main_loop(self):
        ...
        try:
            ...
            for r in results:
                ...
                target_pick_z = (b_point[2]*1000.0) + self.gripper_offset + 20
            ...
            if key == ord('c'):
                ...
                robot_angle = current_view_angle
